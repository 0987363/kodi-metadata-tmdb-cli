...
历史回归已覆盖第二页正确候选、多个标题、跨页重复、空列表、分页异常、254/255 边界及后续页面/标题故障。本轮须复验：模型编号仍先搜索，已知年份不放宽，精确命中后零判断，未命中只判断基本候选；人工明确引用保留直取基本记录与单候选判断。上述新契约已通过受控回归，主分支最终复验与真实样本仍单独验收。
...
下表区分既有实现的历史验证与本轮身份核验、基本作品判断及进程立即失败的整改状态。一次性 CLI、AI 分类、整剧归集和两来源批量取数已有主分支回归证据；本轮变更的集成与 vet 通过；主分支最终 race、构建和真实首部样本复验待完成，不能复用旧通过结论。`res.txt` 固定 10 部节目、`res2.txt` 新集合的实际验收尚未完成。本轮进度由 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md) 管理。
