...
func (j *testJudge) Select(ctx context.Context, r Request, options []Option) (int, error) {
	j.calls++
	j.seen = options
...
func TestResolveKnownIDsSkipSearchAndStopAfterConfirmation(t *testing.T) {
	a, b := &testProvider{name: "tmdb"}, &testProvider{name: "thetvdb"}
	j := new(testJudge)
...
func TestResolveManualSourceConstrainsHintsAndJudgesSingleCandidate(t *testing.T) {
	...
	if a.searchCalls+a.fetchCalls != 0 || b.searchCalls != 0 || j.calls != 1 || got.Work.Ref.ID != "22" {
...
func TestResolveCachesSourceDataButAlwaysJudges(t *testing.T) {
	...
	if p.searchCalls != 2 || p.fetchCalls != 6 {
		t.Fatal("跨语言复用了缓存")
	}
}
