...
本文记录一次性 CLI、AI 媒体分类、多 LLM、独立刮削配置、有序数据源处理、作品身份核验、确认后整剧批量取数、全部搜索正常为空时的 AI 本地整理和 NFO 输出契约。项目只使用电影和剧集两种组织形式，生成面向 Jellyfin、Emby、Silo Server、Kodi 的 NFO；有来源图片时保存配套图片。

具名多 LLM、一次性 `--path` 入口、批量分类、整剧任务和两来源批量取数已有主分支实现与历史 race、vet、平台构建证据。本轮按新契约整改搜索编号交叉核验、基本作品判断和进程立即失败；代码与文档已同步，16 个测试包及 vet 通过；主分支最终 race、vet 与四个平台构建已通过，真实首部遇提取服务 HTTP 503 后立即终止。`res.txt`、`res2.txt` 实际样本验收尚未完成，见 [复验记录](validation-2026-09-24.md)；这两个文件名只标识验证输入，不能成为生产分类规则。执行顺序见 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)，平台依据见 [批量能力实测](provider-batching-research.md)。按用户要求，四端读取采用官方文档与源码核查，不以本地测试冒充服务器实际导入；初始改造记录见 [整改与验收报告](provider-audit.md)。

## 1. 目标与范围

成功标准是：有网站匹配时对应正确作品和单集；全部适用来源的搜索均正常为空时，仅按用户允许的本地输入整理信息并标明依据，未知事实留空。NFO 需符合目标媒体软件的读取契约，来源故障不能伪装成无匹配或 AI 编目成功。
...
| 无匹配内容 | 全部适用来源搜索均正常为空后，通用 LLM 仅依据目录和文件名整理本地元数据，不凭模型知识补写事实 |
...
5. **停止条件**：确认作品后停止来源遍历。只有当前来源的搜索正常为空才能继续下一适用来源；全部适用来源搜索均正常为空才进入受限本地整理。判断拒绝、Jev 低于门槛或任何执行异常，都记录异常并立即终止整个进程，不访问后续来源、作品或本地整理。
6. **确认后取数**：只获取选中来源的完整电影事实，或节目及本轮多季多集事实。节目确认前不获取候选季集；确认后保留既有批量接口、缓存、TMDb 每批 20 个追加对象限制及 TheTVDB 必要单集补取。批量失败或解析缺项直接终止，不能改走逐集请求。
...
请求超时、鉴权失败、限流、网页结构异常、候选非法、判断拒绝/低分、模型响应格式错误、详情/图片/写入错误均立即终止整个进程，不隐式重试，不切换模型。正常未知的可选字段不属于异常。运行边界记录一次脱敏原始异常，`main` 负责非零退出，不在 collector 与 main 重复记错，也不聚合错误后继续处理。正常完成后才汇总结果；已经完成的文件不承诺回滚。

例如来源顺序为 TheTVDB、TMDb：剧集先搜索 TheTVDB；同源同类型编号命中时直接确认，未命中时判断基本候选，只有搜索正常为空才处理 TMDb。电影因 TheTVDB 尚不支持而直接处理 TMDb。只有 TMDb 的模型提示不会越过前面的 TheTVDB；人工限定 TMDb 则只访问 TMDb。
...
| 判断模型确认候选 | 停止来源遍历，获取完整事实后输出 | 后续来源零请求 |
| 判断模型拒绝、Jev 低于门槛或响应异常 | 记录异常，立即终止整个进程 | 不调用后续来源、作品或本地整理 |
| 全部适用来源搜索均正常为空 | 使用通用模型进行受限本地整理 | 输入覆盖、字段边界及本地身份校验通过后才写 NFO |
| 提取、来源、详情、图片或写入异常 | 记录一次脱敏异常并立即非零退出 | 不重试、不继续；已完成文件不自动回滚 |
...
网络、鉴权、协议、网页结构或输入冲突错误仍直接失败；不触发本地整理。已经匹配网站作品后，不能以 AI 填补缺失网站事实。本地整理是用户授权的“全部适用来源搜索正常为空”终点；判断拒绝或任何异常均不能进入该分支。
...
- 原盘结构识别、人工来源及季集/分组约束、目录过滤和响应缓存保留；移除 `tmdb.retry_count`（原网络失败重试次数）及退避、重试统计，因为首次异常必须终止整个进程；定时扫描、文件监听和所有运行模式入口移除，具体字段及原因见第 6 节。
...
- 只有搜索正常空列表可进入下一来源；候选、人工明确引用或选中对象无法获取，来源请求失败、鉴权失败、限流和结构异常均立即终止整个进程。
...
搜索接口/分页的 HTTP 404 表示搜索故障；详情对象的 404 表示对象不存在。两者均终止整个进程，不能转成正常空搜索或换来源。搜索缓存操作名改为 `identity-search-v1`，仅隔离旧的忽略或放宽年份的搜索结果；详情缓存和 `series-*` 整剧缓存的版本不变，不迁移或删除旧缓存。完整分页校验继续保留。

TMDb 正常空第一页允许 `total_pages`（来源总页数）为 0 或 1，但 `total_results`（来源候选总数）必须为 0，且 `results` 必须为显式空列表。某个片名线索没有命中时继续收集其他名称结果，保留已找到的候选；缺字段、负数、后续空页或与非零结果数矛盾的响应仍立即失败。

历史回归已覆盖第二页正确候选、多个标题、跨页重复、空列表、分页异常、254/255 边界及后续页面/标题故障。本轮须复验：模型编号仍先搜索，已知年份不放宽，精确命中后零判断，未命中只判断基本候选；人工明确引用保留直取基本记录与单候选判断。上述新契约已通过受控回归，主分支最终复验已通过；真实首部因提取服务 HTTP 503 尚未完成匹配验收。
...
- `none`、Jev 所选候选未达到 Noul 门槛、响应格式错误、非法候选键和请求故障，均记录异常并立即终止整个进程；不进入下一来源或本地整理。
...
| `tmdb.retry_count` | 原网络失败重试次数；删除，因为首次请求异常必须终止整个进程，同时移除退避和重试统计，旧字段严格拒绝 |
...
| 失败阶段 | 目标约束 |
| --- | --- |
| 提取失败、人工约束冲突、来源或模型请求/协议错误、判断拒绝或低分 | 运行边界记录一次脱敏原始异常，整个进程立即非零退出；不重试、不处理后续来源、作品或本地整理 |
| 当前来源搜索正常为空 | 按顺序尝试下一适用来源；全部适用来源搜索均正常为空后，进入受限本地整理 |
...
下表区分既有实现的历史验证与本轮身份核验、基本作品判断及进程立即失败的整改状态。一次性 CLI、AI 分类、整剧归集和两来源批量取数已有主分支回归证据；本轮变更的集成与 vet 通过；主分支最终 race 与构建通过；真实首部因提取服务 HTTP 503 停止，尚未完成匹配验收，不能复用旧通过结论。`res.txt` 固定 10 部节目、`res2.txt` 新集合的实际验收尚未完成。本轮进度由 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md) 管理。

| 阶段 | 当前状态 | 证据 |
| --- | --- | --- |
| 范围清理 | 已实现 | Kodi RPC、外部媒体工具和音乐视频、旧规则与闲置配置清理；关键词/临时后缀过滤回归 |
| 多模型与刮削配置 | 已实现 | 双模型地址/凭据/模型隔离、未选实例不调用、严格配置及代理校验 |
| 来源顺序、身份核验与基本候选 | 已实现，受控回归通过 | 模型编号先搜索并交叉核验；未命中只判断基本候选；确认后取详情；拒绝/低分终止整个进程 |
| 进程立即失败与配置清理 | 已实现，受控回归通过 | 首次异常记录一次并非零退出，后续来源和作品零调用；删除网络重试配置、退避及重复日志 |
...
| 全部搜索正常为空后的本地整理 | 整理能力已有历史回归；新触发边界待复验 | `DescribeLocal` 限定输入路径，程序生成 `local` 身份；判断拒绝、请求错误和人工明确引用失败均不得进入 |
