...
> 执行时使用 `superpowers:executing-plans` 逐项推进。每一步先完成验证并汇报，通过且没有关键歧义后继续。既有清理、一次性 CLI、AI 分类和整剧批量实现已合并至 master，并有主分支完整复验记录；真实首部样本更换为用户指定的 gpt-5.6-luna 后生成了 local NFO，但正确网站候选被 Jev 拒绝，准确性验收未通过，后续样本暂停。本轮统一整改模型编号搜索核验、仅用基本作品判断、确认后获取季集，以及首次异常终止整个进程。受影响实现及16个测试包、vet已通过；主分支最终 race、vet 和四个平台构建通过；真实首部因 gpt-6-luna 服务 HTTP 503 立即终止，匹配验收仍未完成，历史通过结论不覆盖本轮变更。详见 [复验记录](../../validation-2026-09-24.md)。

**目标：** 清理失效能力，改为从启动参数接收媒体目录并一次性处理；由 AI 判断电影或剧集及作品归属，完成整剧批量刮削或受限本地整理，验证 res.txt 固定样本与 res2.txt 新集合。

**架构：** 沿用现有包结构。程序负责递归发现、明确过滤、去重和原盘结构识别；通用 LLM 批量判断类型、作品归属和季集关系，提供标题、原名/别名、年份及已知网站作品编号提示；程序校验后组织电影或整剧任务。模型提示仍须按来源搜索，命中同来源、同对象类型的真实候选即确认，否则由配置的模型判断基本作品候选。确认后才获取完整电影事实或批量季集：TMDb 使用官方 API 组合资源，TheTVDB 合并公开全集页与季页面，只为批量无法提供的必要字段补取单集页。全部适用来源搜索均正常为空才进行受限本地整理。判断拒绝、低分和任何异常均记录一次并立即终止整个进程。
...
| 作品身份核验与进程立即失败 | 已实现并合并，主分支受控复验通过 | 0902136：搜索核验、基本作品判断、确认后取数、无重试和异常立即退出 |
| 固定 10 部节目真实刮削 | 新规则首部遇 gpt-6-luna 分类服务 HTTP 503 后立即退出 | 服务恢复后重跑同一首部，准确性通过后继续剩余 9 部 |
...
- 通用 LLM 提供识别信息和未验证编号提示；程序以真实搜索结果核验编号，未命中时由所选判断模型判断基本作品。最终网站事实只来自选中来源，确认前不取得候选季集。全部适用来源搜索均正常为空时才允许 AI 本地整理，仅使用目录和文件名信息，未知事实留空。判断拒绝、Jev 低分及任何异常立即终止整个进程，不进入后续来源、作品或本地整理，不隐式重试。
...
| `tmdb.retry_count` | 原网络失败重试次数；删除，因为首次请求异常必须终止整个进程，同时删除退避和重试统计，不接受旧字段或保留无效配置 |
...
flowchart TD
    ...
    V --> R{人工明确作品引用}
    R -->|有| P[直取基本作品记录]
    P --> G[配置的判断器 仅判断基本作品候选]
    R -->|无| D{还有适用来源}
    D -->|有 按配置顺序| E[以片名和年份完整搜索基本作品候选]
    E --> F{搜索正常为空}
    F -->|是| D
    F -->|否| Q{模型编号与候选同来源同类型且相等}
    Q -->|是| H[确认作品 停止来源遍历]
    Q -->|否或编号未知| G
    G -->|接受| H
    G -->|拒绝 低分或异常| X
    D -->|全部搜索正常为空| L[通用 LLM 仅依据本地输入整理]
    H --> I[获取选中作品完整事实 批量取得所需季集]
    I --> N[仅补取批量不能提供的必要字段]
    N --> J[完整性校验 共享输出与逐文件 NFO]
    L --> J
    J --> Z[继续剩余作品 全部成功后退出]
    B -->|异常| X[记录一次脱敏异常 整个进程立即非零退出]
    C -->|异常| X
    V -->|异常| X
    P -->|异常或未找到| X
    E -->|异常| X
...
只有搜索正常空列表才能继续下一适用来源；全部适用来源搜索均正常为空才进入 AI 本地整理。类型不明、约束冲突、非法候选、判断拒绝/低分、模型/请求异常、详情/批量/图片/写入失败均记录一次脱敏原始异常，并立即终止整个进程，不再处理后续来源、作品或本地整理。不切换模型、不隐式重试，不把批量错误转为逐集请求。正常未知的可选字段不是异常；来源确认后的事实缺口不由另一来源或 AI 补齐。

一次运行不驻留、不监听新增文件、不创建定时器。拥有运行边界的代码只记录一次异常，`main` 负责非零退出，移除 collector 与 main 重复记错及聚合失败后继续的行为。正常完成后汇总结果；失败时如实区分已完成、当前失败和尚未处理，不要求后续作品继续运行来闭合成功/失败数量。已经成功的文件不承诺回滚。

### 数据源无匹配后的 AI 本地整理

该分支由用户新增 res2.txt 时明确授权，仅在全部适用来源搜索均正常为空时进入；判断拒绝、低分或任何异常都不能进入。不能预设 res2.txt 全部不存在于网站，也不能跳过已配置且适用的数据源。
...
这一阶段已完成启动参数、配置校验、未分类媒体清单、AI 协议和整剧输出的初次切换；本轮搜索身份核验、基本候选接口和进程立即失败也已实现，并通过完整受控回归与 vet。字段含义以第 3 节为准，来源批量函数只在作品确认后提供完整事实，不保留新旧判断路径。
...
- [x] 模型编号仍先按标题与年份搜索；已知年份不追加无年份查询，TheTVDB 完整分页后按返回年份核验；仅隔离旧搜索缓存，详情与整剧缓存版本不改。只有同来源、同类型编号命中真实候选才零判断确认。未知或未命中时基本候选交给判断器，单候选也判断；人工明确引用仍直取基本记录并判断，不能被模型覆盖。首源确认后，后续来源请求为零。
- [x] 本地整理只在全部适用来源搜索均正常为空时触发，复用通用模型实例。首源空搜索可继续；判断拒绝、低分、请求错误和人工指定失败均立即终止整个进程，后续来源、作品及本地整理调用为零。
- [x] 移除网络重试配置、退避与重试统计，旧配置字段严格拒绝；首次任意请求异常仅调用一次。collector 返回首个错误而不聚合后继续，运行边界只记一次脱敏原始异常，main 非零退出且不重复记错。
...
- [ ] 请求计数的受控回归已通过：缓存命中不计外发请求，已删除重试统计；编号命中后零判断、确认前零季集、首个异常后零后续请求已验证。真实首部在分类阶段被 503 阻塞，实际网站请求效率、详情补取次数和原因仍待样本阶段核验。
- [x] 观测归属限定在任务编排与请求执行边界；纯模型、常量和协议结构不输出日志。复用现有日志入口，原始异常只由拥有运行边界的代码脱敏记录一次，main 只负责非零退出；验证无重复错误、无凭据泄漏和无聚合失败后继续处理。
...
- [ ] 新契约完整受控复验通过后，按固定样本顺序先重新验证第一部，准确匹配与输出通过后继续剩余九部。任意异常或判断拒绝/低分立即停止，不用后续作品继续消耗请求，也不换模型、来源顺序或样本掩盖问题。
...
- [ ] 类型和归组通过校验后，tv 进入整剧批量任务，movie 进入独立作品任务；不能把有编号自动视为剧集或把无编号自动视为电影。来源匹配使用网站事实，所有适用来源搜索均正常为空才进行本地整理；判断拒绝或低分直接终止进程。
...
| 首个来源确认 | 后续来源零请求，确认后才批量取数，输出保持同一来源 |
| 首个来源搜索正常为空 | 允许下一适用来源；不调用空候选判断 |
| 全部适用来源搜索均正常为空 | 通用 LLM 基于输入批量整理本地元数据；未知事实留空，使用程序生成的本地身份 |
| 判断拒绝或 Jev 低于门槛 | 记录一次异常，立即非零退出；下一来源、下一作品、本地整理零调用 |
| 任意鉴权/网络/协议/详情故障或人工指定失败 | 第一次失败即终止进程，无重试、无后续来源或作品、无本地整理 |
| 第一部作品失败且仍有第二部 | 第二部不执行；日志保留一次脱敏原始异常，main 不重复记错 |
...
当前方案保留既有一次性 CLI、AI 分类、整剧批量及 NFO 范围，并按最新契约统一了搜索核验、基本作品判断、确认后取数、全空搜索进入本地整理和首个异常终止的端到端分支。代码与文档正同步整改；新实现的集成、race、vet、构建和首部真实样本复验待完成，两组真实样本整体尚未通过。历史主分支和初次批量实现的测试结果不能替代本轮验证。
