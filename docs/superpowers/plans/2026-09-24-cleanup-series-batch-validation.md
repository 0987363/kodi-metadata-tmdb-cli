...
> 执行时使用 `superpowers:executing-plans` 逐项推进。每一步先完成验证并汇报，通过且没有关键歧义后继续；截至本次提交，阶段一至三已实现并通过完整测试；按最新要求，先提交并合并到 master，再在主分支进行阶段四最终复验，随后执行阶段五真实样本。主分支复验已通过；真实样本首部更换为用户指定的 gpt-5.6-luna 后完成输出，但正确网站候选被 Jev 拒绝而生成 local NFO，准确性验收未通过，后续样本暂停。详见 [复验记录](../../validation-2026-09-24.md)。
...
| 固定 10 部节目真实刮削 | 首部输出完成，但 Jev 拒绝正确候选，准确性未通过 | 确认判断整改路径后复验首部，再继续剩余 9 部 |
...
主分支 `6161a7e` 复验：16 个测试包、197 个测试函数、526 个测试节点，race 零失败；vet 与 macOS、Linux amd64/arm64、Windows amd64 构建通过。文档失效链接已修正，本地 NFO 读取边界见专项报告。
...
- [x] 请求计数由实际请求执行边界累计，节目流程汇总提取批次、判断次数、来源批次及详情补取原因；验证器汇总成报告。缓存命中不能计为外发请求，重试与首次请求分别计数；统计发现数量、明确排除、AI 分类与未分类、归组、输出成功与失败，保证数量闭合。
- [x] 观测归属限定在任务编排与请求执行边界；纯模型、常量和协议结构不输出日志。复用现有日志入口，不以此次验收为理由迁移整个日志系统；只记录脱敏路径或来源信息、阶段、数量、耗时及原始错误，不输出认证参数。
- [x] 单文件原子替换边界不变；后续图片或文件写入失败时报告失败，不承诺整部剧多文件事务回滚。
- [x] 全量 race、vet、平台构建、正式文档与方案的本地链接检查通过，更新 README 中的真实行为和仍未覆盖的验收范围。
