...
> 执行时使用 `superpowers:executing-plans` 逐项推进。每一步先完成验证并汇报，通过且没有关键歧义后继续。既有清理、一次性 CLI、AI 分类和整剧批量实现已合并至 master，并有主分支完整复验记录；真实首部样本更换为用户指定的 gpt-5.6-luna 后生成了 local NFO，但正确网站候选被 Jev 拒绝，准确性验收未通过，后续样本暂停。本轮统一整改模型编号搜索核验、仅用基本作品判断、确认后获取季集，以及首次异常终止整个进程。受影响实现及16个测试包、vet已通过；主分支最终race、构建及首部真实样本复验仍待完成，历史通过结论不覆盖本轮变更。详见 [复验记录](../../validation-2026-09-24.md)。
...
| 作品身份核验与进程立即失败 | 新契约已确定，代码与文档同步整改中 | 搜索核验、基本作品判断、确认后取数、无重试和异常立即退出；完整复验待完成 |
| 固定 10 部节目真实刮削 | 历史首部输出完成，但 Jev 拒绝正确候选，准确性未通过 | 按本轮新契约复验首部，通过后继续剩余 9 部 |
...
历史主分支提交 `6161a7e` 的复验为：16 个测试包、197 个测试函数、526 个测试节点，race 零失败；vet 与 macOS、Linux amd64/arm64、Windows amd64 构建通过。这是本轮身份与立即失败整改前的基线；新实现必须重新执行本阶段检查，以下未勾选项仍待完成。本地 NFO 读取边界见专项报告。
...
- [ ] 请求计数由实际请求执行边界累计，记录提取批次、判断次数、来源批次及详情补取原因；缓存命中不计为外发请求，删除重试统计。验证编号命中搜索但零判断、确认前零季集、首个异常后零后续请求；区分发现、排除、已分类、已完成、当前失败和尚未处理。
- [ ] 观测归属限定在任务编排与请求执行边界；纯模型、常量和协议结构不输出日志。复用现有日志入口，原始异常只由拥有运行边界的代码脱敏记录一次，main 只负责非零退出；验证无重复错误、无凭据泄漏和无聚合失败后继续处理。
- [ ] 单文件原子替换边界不变；首个图片或文件写入失败立即终止整个进程，后续作品零处理；已成功文件保留，不承诺多文件事务回滚。
- [ ] 对本轮新实现执行全量 race、vet、平台构建、实际 CLI 受控回归、正式文档与方案本地链接检查；验证通过后记录新证据，不沿用历史结果。README 同步当前契约及仍未覆盖的真实样本与服务器导入边界。
