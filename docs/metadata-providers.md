...
具名多 LLM、顺序来源与完整候选获取、一次性 `--path` 入口、未预设类型的批量分类、整剧任务、两来源批量取数和正常无匹配后的受限本地整理均已写入当前代码。最终完整回归及 `res.txt`、`res2.txt` 实际样本运行仍待完成；这两个文件名只标识验证输入，不能成为生产分类规则。执行顺序见 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)，平台依据见 [批量能力实测](provider-batching-research.md)。按用户要求，四端读取采用官方文档与源码核查，不以本地测试冒充服务器实际导入；初始改造记录见 [整改与验收报告](provider-audit.md)。
...
下表区分初始改造的历史验证与当前工作树的实现状态。一次性 CLI、AI 分类、整剧归集、批量模型协议、两来源批量取数、无匹配后的 AI 本地整理已写入代码；最终完整回归与 `res.txt` 固定 10 部节目、`res2.txt` 新集合的实际运行仍待完成。本轮进度由 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md) 管理。
...
| 一次性 CLI、AI 分类及整剧批量取数 | 代码已实现，最终回归待完成 | `--path` 单目录、最多 50 文件一批、整剧任务与来源批量接口；尚不能以历史验证计数证明本轮全部通过 |
| 正常无匹配后的本地整理 | 代码已实现，最终回归待完成 | `DescribeLocal` 限定输入路径，程序生成 `local` 身份；来源错误和人工明确引用失败不会转入此路径 |
