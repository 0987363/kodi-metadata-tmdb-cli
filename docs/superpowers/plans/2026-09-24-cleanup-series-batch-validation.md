...
执行时使用 `superpowers:executing-plans` 逐项推进，每步先验证并汇报，通过且无关键歧义后继续。既有清理、单次 CLI、AI 分类、整剧批量与身份核验已合并；当前修订只调整三方式接续及其日志、验证契约。前两部固定真实样本共 52 个视频已通过；Super.Science 在旧的首源拒绝即退出规则下未访问 TMDb 和 AI。修正后从该部 8 个视频继续，详见 [复验记录](../../validation-2026-09-24.md)。
...
| 固定 10 部节目真实刮削 | 前两部 52 个视频通过；第三部被旧的首源拒绝即退出规则阻断 | 修正接续规则后从 Super.Science 的 8 个视频继续 |
...
- [ ] 新接续契约的完整受控复验通过后，从 Super.Science 继续固定样本顺序。前两部已有真实验证证据；后续样本不重新抽取，不更换模型、来源顺序或门槛来掩盖问题。单源失败继续下一方式，全部方式失败或共同输入/输出错误才停止。
...
当前方案保留一次性 CLI、AI 分类、搜索身份核验、基本作品判断、整剧批量及 NFO 范围。来源确认后完整取数，失败继续下一来源；网站来源耗尽后进入输入限定 AI 本地整理，AI 也失败才以元数据获取失败退出。共同输入、人工硬约束、取消与输出边界保持。当前修订已通过 16 包、235 个测试函数、654 个测试节点的 race、vet 和四个平台构建；独立审查、主分支合并复验和 Super.Science 实测按顺序完成。两组真实样本整体尚未通过。
