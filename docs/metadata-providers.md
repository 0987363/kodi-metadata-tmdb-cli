...
具名多 LLM、一次性 `--path` 入口、批量分类、整剧任务、编号搜索核验和基本候选判断均已有实现。本轮只修正三方式接续：网站来源未成功继续下一方式，AI 本地整理也失败才以元数据获取失败退出。真实固定样本前两部 52 个视频已通过，第三部 Super.Science 待新流程复验；完整样本仍未验收。见 [复验记录](validation-2026-09-24.md) 与 [执行方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)。[批量能力实测](provider-batching-research.md)和四端官方文档/源码核查边界保持，未做服务器实际导入。
...
历史回归已覆盖第二页正确候选、多个标题、跨页重复、空列表、分页异常、254/255 边界及后续页面/标题故障。本轮须复验：模型编号仍先搜索，已知年份不放宽，精确命中后零判断，未命中只判断基本候选；人工明确引用保留直取基本记录与单候选判断。身份核验契约已有受控回归与前两部真实样本证据；本轮接续规则已通过当前分支 race、vet 和四个平台构建：16 包、235 个测试函数、654 个节点；主分支及真实样本单独复验。
...
下表区分既有能力与本轮接续规则。历史测试不替代当前修订的回归；真实前两部共 52 个视频通过，第三部 Super.Science 需用新流程复验。res.txt 固定 10 部及 res2.txt 的整体验收仍未完成，进度见 [执行方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)。
...
| 三方式接续与配置清理 | 当前分支受控回归通过，合并后复验 | 单个来源失败继续，网站来源和 AI 全部未成功才以元数据失败退出；不恢复网络重试配置 |
