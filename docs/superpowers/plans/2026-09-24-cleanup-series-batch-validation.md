...
执行时使用 `superpowers:executing-plans` 逐项推进，每步先验证并汇报，通过且无关键歧义后继续。既有清理、单次 CLI、AI 分类、整剧批量与身份核验已合并；当前修订只调整三方式接续及其日志、验证契约。前三部固定真实样本共 60 个视频已通过；Super.Science 已完成双网站拒绝后 AI 整理，第四部 104 个视频因未知季集号暂停，详见 [复验记录](../../validation-2026-09-24.md)。
...
| 固定 10 部节目真实刮削 | 前三部 60 个视频通过；第四部 104 个因未知季集号停止 | 核实分类结果与输入分段关系，尚未继续后六部 |
| res2.txt 本地内容集合 | 529 个视频已完整复现，真实运行等待首部准确性问题处理 | 529 个视频，覆盖相声、评书、专场、年度目录和分段文件；验证无匹配后的 AI 本地整理 |
...
- [ ] 新接续契约的完整受控复验通过后，从 Super.Science 继续固定样本顺序。前三部已有真实验证证据；第四部季集输入待核实，后续样本不重新抽取，不更换模型、来源顺序或门槛来掩盖问题。单源失败继续下一方式，全部方式失败或共同输入/输出错误才停止。
...
该组增加本地编目边界验证，不替换 A 组固定样本。已解析并复现 529 个视频，忽略 5 个非视频文件；因首部判断准确性问题，本组尚未执行网站检索或调用 LLM；“几乎找不到”是用户提供的样本特征，实际是否无匹配由运行证据判断。
...
当前方案保留一次性 CLI、AI 分类、搜索身份核验、基本作品判断、整剧批量及 NFO 范围。来源确认后完整取数，失败继续下一来源；网站来源耗尽后进入输入限定 AI 本地整理，AI 也失败才以元数据获取失败退出。共同输入、人工硬约束、取消与输出边界保持。当前修订已通过 16 包、235 个测试函数、654 个测试节点的 race、vet 和四个平台构建；独立审查、主分支合并复验和 Super.Science 实测已完成；固定前三部 60 个视频通过，第四部因未知季集号停止。两组真实样本整体尚未通过。
