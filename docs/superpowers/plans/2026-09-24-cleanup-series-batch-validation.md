...
执行时使用 `superpowers:executing-plans` 逐项推进，每步先验证并汇报，通过且无关键歧义后继续。既有清理、单次 CLI、AI 分类、整剧批量与身份核验已合并；当前修订只调整三方式接续及其日志、验证契约。前三部固定真实样本共 60 个视频已通过；Super.Science 已完成双网站拒绝后 AI 整理，第四部 104 个视频按用户要求暂时跳过；随后 Friends 的 234 个视频因合并集协议限制停止，详见 [复验记录](../../validation-2026-09-24.md)。
...
| 固定 10 部节目真实刮削 | 前三部 60 个通过；第四部 104 个暂时跳过；Friends 的 234 个因合并集协议停止 | 等待确定多集文件处理方向，其余五部 172 个未运行；跳过不计通过 |
| res2.txt 本地内容集合 | 529 个视频已完整复现，真实运行等待 A 组剩余样本验证 | 529 个视频，覆盖相声、评书、专场、年度目录和分段文件；验证无匹配后的 AI 本地整理 |
...
- [ ] Super.Science 已通过接续复验；按用户要求暂时跳过第四部整部 104 个视频，从 Friends 继续固定顺序。Friends 已运行并因合并集协议限制停止，等待确定后续方向。前三部保留既有通过证据，第四部不计通过，后续样本不重新抽取，不更换模型、来源顺序或门槛来掩盖问题。单源失败继续下一方式，全部方式失败或共同输入/输出错误才停止。
- [ ] 每部包含目录内所有原始视频，继续应用现有过滤规则。记录“输入 570、按规则排除、实际纳入、已成功、当前失败、尚未处理”的计数，不默认每个输入都应输出 NFO。
...
该组增加本地编目边界验证，不替换 A 组固定样本。已解析并复现 529 个视频，忽略 5 个非视频文件；本组尚未执行网站检索或调用 LLM，待 A 组剩余样本后继续；“几乎找不到”是用户提供的样本特征，实际是否无匹配由运行证据判断。
...
- [ ] 类型和归组通过校验后，tv 进入整剧批量任务，movie 进入独立作品任务；不能把有编号自动视为剧集或把无编号自动视为电影。网站来源完整成功使用网站事实，全部网站来源未成功后执行受限 AI 本地整理；AI 也失败才终止元数据获取。
...
当前方案保留一次性 CLI、AI 分类、搜索身份核验、基本作品判断、整剧批量及 NFO 范围。来源确认后完整取数，失败继续下一来源；网站来源耗尽后进入输入限定 AI 本地整理，AI 也失败才以元数据获取失败退出。共同输入、人工硬约束、取消与输出边界保持。当前修订已通过 16 包、235 个测试函数、654 个测试节点的 race、vet 和四个平台构建；独立审查、主分支合并复验和 Super.Science 实测已完成；固定前三部 60 个视频通过，第四部 104 个视频按用户要求暂时跳过，Friends 的 234 个视频因合并集协议限制停止，其余五部 172 个待运行。两组真实样本整体尚未通过。
