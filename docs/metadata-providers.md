...
本文记录已实现的一次性 CLI、AI 媒体分类、多 LLM、独立刮削配置、有序数据源处理、整剧批量取数、无匹配时的 AI 本地整理和 NFO 输出契约。项目只使用电影和剧集两种组织形式，生成面向 Jellyfin、Emby、Silo Server、Kodi 的 NFO；有来源图片时保存配套图片。

具名多 LLM、顺序来源与完整候选获取、一次性 `--path` 入口、未预设类型的批量分类、整剧任务、两来源批量取数和正常无匹配后的受限本地整理均已写入当前代码。主分支完整 race、vet 和平台构建已通过，`res.txt`、`res2.txt` 实际样本验收尚未完成，见 [复验记录](validation-2026-09-24.md)；这两个文件名只标识验证输入，不能成为生产分类规则。执行顺序见 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)，平台依据见 [批量能力实测](provider-batching-research.md)。按用户要求，四端读取采用官方文档与源码核查，不以本地测试冒充服务器实际导入；初始改造记录见 [整改与验收报告](provider-audit.md)。

## 1. 目标与范围

成功标准是：有网站匹配时对应正确作品和单集；没有网站匹配时，仅按用户允许的本地输入整理信息并标明依据，未知事实留空。NFO 需符合目标媒体软件的读取契约，来源故障不能伪装成无匹配或 AI 编目成功。
...
| 无匹配内容 | 全部适用来源正常完成且无确认匹配后，通用 LLM 仅依据目录和文件名整理本地元数据，不凭模型知识补写事实 |
...
```mermaid
flowchart TD
    A[启动参数传入媒体目录] --> A1[程序递归发现 明确过滤 去重 原盘识别]
    A1 --> B[通用 LLM 批量分类并提取]
    B --> B1[校验类型 覆盖和目录 组织电影或整剧任务]
    B1 --> C[应用人工约束]
    C --> D{还有适用来源}
    D -->|有，按配置顺序| E[直取或搜索 获取作品事实及批量季集摘要]
    D -->|全部正常完成但无匹配| I[通用 LLM 依据本地输入整理 未知留空]
    I --> J
    I -->|输入不足或执行错误| K
    E --> F{有有效候选}
    F -->|无| D
    ...
    G --> H{确认匹配}
    H -->|否| D
    H -->|是| L[停止来源遍历 批量取得所需详情]
    L --> M[仅补取批量无法提供的必要字段]
    M --> J[校验完整性 共享输出一次 单集逐文件输出]
    B -->|执行错误| K[记录失败]
    B1 -->|校验失败| K
    C -->|约束冲突| K
    E -->|执行错误| K
...
1. **发现与分类**：程序只扫描启动参数指定的目录，递归发现并去重媒体，不按目录预设电影/剧集。通用 LLM 每批最多处理 50 个媒体单元，返回类型、作品线索与明确季集；类型不明、漏项、路径越界或分组冲突均报错，不用规则补判。程序校验后才组织电影或整剧任务，后续处理器不再重复进行预设类型的提取。
2. **约束**：人工指定来源、网站作品编号、季和分组优先约束可访问来源及候选。季与分组设置仍按原目录作用域应用。人工指定不跳过提取和判断，也不允许为继续尝试而忽略人工约束。
3. **候选取数**：按配置顺序访问来源，跳过不支持当前媒体类型或被人工约束排除的来源。有当前来源的节目或电影记录编号则直取，没有则完整搜索。剧集取得节目事实与所需季集的批量摘要，不提前获取所有候选的完整图库与演职员。一个来源的编号不能定位另一个来源。
4. **判断**：取得当前来源的真实事实后，调用刮削设置指定的候选判断 LLM。同一剧集任务、同一有候选来源只判断一次，使用整剧上下文及涉及季集的去重证据。即使只有一个候选也要判断。
5. **停止条件**：确认一个候选后立即停止，不访问后续来源。来源明确未找到，或模型明确未接受任何候选，才进入下一适用来源；全部允许且适用来源正常完成仍无匹配，才进入受限的 AI 本地整理。人工明确指定作品失败不进入该分支。
6. **完整详情**：确认后仅获取所选来源的剩余详情，优先使用已核验的批量能力；只有必要字段确实不能批量取得时才补取单集页。批量请求失败或解析缺项不能改走逐集请求；单个来源明确为空的可选字段不触发无限补取。
7. **本地整理**：仅复用 `scraper.extract_llm` 指定的通用实例；`DescribeLocal` 每批最多处理 50 个输入路径，只返回由目录和文件名明确支持的标题、简介及分类，未知留空。季集坐标来自先前已校验的分类结果或人工约束，程序生成本地对象标识。不调用 Jev 选择虚构来源；具体契约见本节下文。
8. **输出**：本轮映射和必需信息校验通过后，节目 NFO 与共有图片按整剧处理一次，单集 NFO 和缩略图按原视频路径输出。同一来源单集的多个本地版本共享事实；本地整理没有来源图片时不下载或生成图片。模型不能指定输出位置。

请求超时、鉴权失败、限流、网页结构异常和模型响应格式错误属于执行错误，不当作未匹配继续尝试来源或本地整理。模型不会自动切换另一个配置实例。运行不驻留、不监听、不定时扫描；结束时汇总成功、排除与失败，存在处理失败时返回非零退出码。
...
| 输入或结果 | 后续动作 | 输出边界 |
| --- | --- | --- |
| 缺少媒体路径或目录不可读 | 明确失败，不默认当前目录 | 无模型/来源请求 |
| AI 类型未判定、覆盖不完整或目录归组冲突 | 报告识别失败，不用名称规则重判 | 无网站请求或 NFO 输出 |
...
| 全部适用来源正常完成且未确认匹配 | 使用通用模型进行受限本地整理 | 输入覆盖、字段边界及本地身份校验通过后才写 NFO |
...
网络、鉴权、协议、网页结构或输入冲突错误仍直接失败；不触发本地整理。已经匹配网站作品后，不能以 AI 填补缺失网站事实。本地整理是用户授权的“来源正常无匹配”终点，不是来源错误处理策略。
...
- 原盘结构识别、人工来源及季集/分组约束、目录过滤、响应缓存和有限重试保留；定时扫描、文件监听和所有运行模式入口移除，具体字段及原因见第 6 节。
...
| `Option.Episodes` | 已替代原先只绑定一个目标单集的 `Option.Episode`，保存当前节目候选涉及的同来源单集事实集合，以支持多季多集统一判断；电影候选不含单集事实 |
...
通用 LLM 返回的各条目中的 `tmdb_id` 表示电影或节目在 TMDb 的记录编号；`thetvdb_id` 表示节目在 TheTVDB 的记录编号。两者使用正整数字符串，未知留空，不能填演员、季或单集编号。它们不会未经验证就写入 NFO。
...
- 当前实现逐来源获取、判断、确认即停止；整剧任务和来源批量取数已接入，没有跨来源汇总模式开关。
...
搜索接口/分页的 HTTP 404 表示搜索故障，不转成未匹配；详情对象的 404 仍表示对象不存在。既有搜索缓存键及响应封装版本为 3，旧首屏缓存不会被复用为完整集合；整剧批量结果另用 `series-*` 命名空间。
...
来源响应缓存在 `.metadata/cache/`，缓存身份包含来源配置、对象类型、来源引用、语言、分组和季集坐标。整剧批量结果使用独立的 `series-*` 命名空间和键版本，不需要仅因这一改动统一提高旧响应封装版本。图片来源记录在 `.metadata/artwork/`。人工来源文件与已有人工文本设置不属于可清理响应缓存。

只缓存来源事实，命中缓存仍执行本轮提取和判断；剧集的“本轮”是整剧任务，不是每个视频。来源在内存中的快照只在单次 CLI 运行内共用；`scraper.cache_hours` 控制跨次运行的磁盘缓存有效期，`0` 禁用磁盘缓存。批量路径保持来源与语言隔离，不缓存模型判断结果。

| 失败阶段 | 目标约束 |
| --- | --- |
| 提取失败、人工约束冲突、来源或模型请求/协议错误 | 返回错误；不进入 NFO 写入，也不作为未匹配跳到下一来源 |
| 当前来源无候选、模型明确拒绝或 Jev 未达到匹配门槛 | 按顺序尝试下一适用来源；不选首条、不切换判断模型；全部适用来源正常完成且无匹配后，进入受限本地整理 |
...
下表区分初始改造的历史验证与当前工作树的实现状态。一次性 CLI、AI 分类、整剧归集、批量模型协议、两来源批量取数、无匹配后的 AI 本地整理已写入代码；主分支最终回归已经通过，`res.txt` 固定 10 部节目、`res2.txt` 新集合的实际验收尚未完成。本轮进度由 [清理、整改与验证方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md) 管理。
...
| 一次性 CLI、AI 分类及整剧批量取数 | 代码已实现，主分支回归通过 | `--path` 单目录、最多 50 文件一批、整剧任务与来源批量接口；本轮 race 为 16 个测试包、197 个测试函数、526 个测试节点 |
| 正常无匹配后的本地整理 | 代码已实现，主分支回归通过 | `DescribeLocal` 限定输入路径，程序生成 `local` 身份；来源错误和人工明确引用失败不会转入此路径 |
