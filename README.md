...
The merged implementation has controlled regression, race, vet and build evidence. The remaining sample runs are complete: across 1,099 sample videos, 149 passed output checks, 403 failed layout checks, 209 were blocked by execution failures, and 338 were temporarily skipped at the user’s request. Overall acceptance has not passed. See the [latest validation report](docs/validation-2026-09-25.md) and [known issues](docs/known-issues.md). This validation pass recorded issues without changing production rules, models or thresholds. Empty-video checks do not establish actual imports into the four media servers.
