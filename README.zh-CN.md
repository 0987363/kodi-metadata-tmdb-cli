...
- 按配置顺序搜索并核验作品；模型编号命中同来源、同类型的真实搜索候选时跳过判断，否则判断基本作品候选。完整分页与候选去重保留，首次异常立即终止整个进程。
...
- 按整剧任务批量获取来源季集事实；全部适用来源搜索均正常为空时，只依据目录和文件名整理本地标题、简介与分类，并由程序生成本地标识。
- 保留明确目录、关键词与临时后缀过滤、DVD/Blu-ray 目录处理及人工来源与季集约束；完善事实缓存、原子写入、错误传播、代理隔离与连接超时。

仅支持电影和剧集的命令行元数据工具，NFO 输出面向 Jellyfin、Emby、Silo Server 和 Kodi。流程为 **单目录发现 → 通用 LLM 批量分类提取 → 电影或整剧任务 → 真实基本搜索候选 → 编号交叉核验或模型判断 → 选中作品及季集详情 → NFO 与来源图片**。只有全部适用来源搜索均正常为空时，才进入受限本地整理。工具不探测媒体流、不从视频截图。
...
5. 确认作品后停止来源遍历，再获取完整电影事实，或节目及本轮需要的多季多集事实；保留既有批量接口、缓存、TMDb 每批 20 个追加对象限制和 TheTVDB 必要详情补取，最后校验并写入 NFO/图片。
6. 只有搜索正常空列表可进入下一适用来源，全部适用来源搜索均正常为空才整理本地信息。判断拒绝、Jev 低分或任何提取、请求、协议、详情、图片、写入异常，均记录异常并立即终止整个进程，不重试、不继续后续来源或作品、不进入本地整理。

来源顺序就是优先顺序；人工指定来源限制可访问范围，人工明确作品引用保留硬约束。电影目前仅由 TMDb 支持，TheTVDB 使用电视剧和单集的公开 HTML。正常未知的可选字段保持空值，不属于异常。

运行边界只记录一次脱敏原始异常，`main` 负责非零退出；collector 与 main 不重复记错，不通过汇总失败继续处理后续作品。已完成文件不自动回滚。
...
`--path` 必须且只能指定一次现有目录；位置参数、重复 `--path`、旧 `-mode` 均报错，不默认扫描当前目录。处理结束即退出，不监听或定时扫描。`-config` 指定配置文件，默认优先从工作目录读取，再从可执行文件目录读取；`-version` 与帮助入口保留。首次异常立即终止整个进程并返回非零退出码，后续作品不再处理，先前成功写入的文件不自动回滚。
...
- 所有适用来源搜索均正常为空时，通用模型的 `DescribeLocal` 只依据输入路径返回标题、简介及分类；未知简介和分类留空。程序为电影、节目和单集生成稳定的 `local` 标识，不伪造网站编号、来源页面或图片。判断拒绝、Jev 低分、HTTP/协议异常及人工明确指定记录失败均立即终止整个进程，不触发本地整理。
...
一次性 CLI、批量分类、整剧取数和受限本地整理已有主分支历史 race、vet 与四个平台构建证据。本轮身份交叉核验、基本作品判断和进程立即失败已实现，16 个测试包及 vet 通过；主分支最终 race（218 个测试函数、594 个测试节点）与四个平台构建通过。首部真实样本遇到提取服务 HTTP 503 后立即终止，真实匹配仍未验收，不能沿用历史通过结论。`res.txt`、`res2.txt` 实际样本验收尚未完成，当前进展见 [复验记录](docs/validation-2026-09-24.md)。上述文件名只标识验证输入；受控回归不代表真实模型准确率或四种媒体软件实际导入已验证。所选判断服务与通用 LLM 需使用你的账户凭据验证；中文媒体判断效果应按实际样本检查。
