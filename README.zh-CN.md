...
一次性 CLI、批量分类、整剧取数和受限本地整理已有主分支历史 race、vet 与四个平台构建证据。本轮身份交叉核验、基本作品判断和进程立即失败已实现，16 个测试包及 vet 通过；主分支最终 race、构建及首部真实样本复验待完成，不能沿用历史通过结论。`res.txt`、`res2.txt` 实际样本验收尚未完成，当前进展见 [复验记录](docs/validation-2026-09-24.md)。上述文件名只标识验证输入；受控回归不代表真实模型准确率或四种媒体软件实际导入已验证。所选判断服务与通用 LLM 需使用你的账户凭据验证；中文媒体判断效果应按实际样本检查。
