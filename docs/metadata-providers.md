...
具名多 LLM、一次性 `--path` 入口、批量分类、整剧任务、编号搜索核验和三方式接续均已有实现。本轮其余样本已执行完毕，但发现节目归组、一文件多集、前置校验等问题，整体尚未验收通过；目标契约与当前缺陷分别记录。见[最新续验报告](validation-2026-09-25.md)、[待整改问题](known-issues.md)和[执行方案](superpowers/plans/2026-09-24-cleanup-series-batch-validation.md)。[平台批量能力实测](provider-batching-research.md)及四端官方文档/源码核查边界保持，未做服务器实际导入。
...
下表区分既有受控验证与真实样本结果。本轮其余集合全部已有运行记录，累计输出校验149个通过、403个布局失败、209个执行失败、338个用户暂跳；整体尚未验收通过。具体边界及待核查事项见[最新报告](validation-2026-09-25.md)与[问题清单](known-issues.md)。
...
| `res.txt`、`res2.txt` 实际样本 | 已执行本轮非跳过集合，存在未通过项 | 保留独立退出码、字段与布局校验；问题先记录，生产规则未改 |
