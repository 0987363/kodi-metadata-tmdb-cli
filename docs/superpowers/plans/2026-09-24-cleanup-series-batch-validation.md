...
执行时使用既有计划逐项推进。当前代码已包含一次性CLI、AI分类、整剧批量、身份核验与三方式接续。本轮按用户要求先记录问题、继续其他样本：新增执行701个视频，保留此前60个通过证据，338个按要求暂跳。两组非跳过集合均已有运行结果，但整体尚未验收通过。见[最新续验报告](../../validation-2026-09-25.md)和[待整改问题](../../known-issues.md)。

**目标：** 清理失效能力，改为从启动参数接收媒体目录并一次性处理；由 AI 判断电影或剧集及作品归属，完成整剧批量刮削或受限本地整理，验证 res.txt 固定样本与 res2.txt 新集合。

**架构：** 沿用现有包结构。程序负责递归发现、明确过滤、去重和原盘结构识别；通用 LLM 批量判断类型、作品归属和季集关系，提供标题、原名/别名、年份及已知网站作品编号提示；程序校验后组织电影或整剧任务。模型提示仍须按来源搜索，命中同来源、同对象类型的真实候选即确认，否则由配置的模型判断基本作品候选。确认后才获取完整电影事实或批量季集：TMDb 使用官方 API 组合资源，TheTVDB 合并公开全集页与季页面，只为批量无法提供的必要字段补取单集页。全部适用网站来源都未取得可用元数据后进行受限本地整理；逐源记录搜索、判断和完整事实获取失败原因。只有网站来源和 AI 本地整理全部未成功，才以元数据获取失败退出。共同的输入/分类校验、人工硬约束、用户取消及输出失败仍终止。
...
| 失效仓库文件清理 | 已随既有清理合并 | 保留历史清理结果，不重复执行 |
...
| 作品身份核验与三方式接续 | 身份核验已有实现，本轮修正过早退出 | 按来源完整尝试，网站来源均未成功后进入 AI 本地整理 |
| 固定10部节目真实刮削 | 本轮非跳过样本均已执行 | 输出校验134个通过、85个布局失败、13个运行失败、338个用户暂跳；其中14个本地输出的网站匹配待核 |
| res2.txt本地内容集合 | 529个视频已按8个集合执行 | 输出校验15个通过、318个布局失败、196个分类超时；其中3个本地输出的网站匹配待核 |

整合后的约束：

- 仅支持电影和剧集。ffmpeg、ffprobe、音乐视频、Kodi JSON-RPC、匹配/搜索模式和规则回退继续移除。
- 通用 LLM 提供识别信息和未验证编号提示；程序以真实搜索结果核验编号，未命中时由所选判断模型判断基本作品。最终网站事实只来自选中来源，确认前不取得候选季集。全部适用网站来源均未取得可用元数据时允许 AI 本地整理，仅使用目录和文件名信息，未知事实留空。单个来源无结果、判断拒绝/低分或取数故障均记录后继续下一方式；只有适用网站来源与 AI 本地整理都失败才终止元数据获取，不隐式重试。共同输入、人工约束和输出错误仍终止。
...
| `tmdb.retry_count` | 原网络失败重试次数；删除，因为不对同一失败请求隐式重试，改由编排层尝试下一获取方式，同时删除退避和重试统计，不接受旧字段或保留无效配置 |
...
| `items[].tmdb_id` | 该条目所属电影或节目在 TMDb 的未验证记录编号提示，类型取决于已返回的 media_type；可基于明确标记或模型已有知识给出，未知留空，不强制编造，不允许填演员、季或单集编号；用途是与真实搜索候选交叉核验 |
| `items[].thetvdb_id` | tv 条目所属节目在 TheTVDB 的未验证记录编号提示，用途是与真实搜索候选交叉核验；movie 与未知值留空，不允许填季或单集编号 |
| `Candidate` | 基本作品候选：来源作品引用、标题、原名、年份；复用现有结构，不添加季集、剧情、演员、图库或推测字段 |
| `Judge.Select` | 判断接口改收 `[]Candidate`，替代完整详情集合 `[]Option`；原因是判断阶段只能确认作品，不能携带或提前获取季集事实 |
| `Option.Work` | 确认后的完整电影或节目事实，归属详情获取和输出阶段，不再用于候选判断 |
| `Option.Episodes` | 替代原只绑定一个目标单集的 `Option.Episode`，保存选中节目本轮涉及的同来源单集事实集合，以支持整剧输出；仅确认后获取，电影为空集合 |
...
人工明确电影/节目引用直取基本作品记录，必须交给配置的判断器做单候选判断，模型编号提示不能覆盖该引用或跳过判断。仅限定来源时仍在该来源搜索。人工明确引用未找到、对象类型或分组约束冲突，立即终止整个进程。

其他情况按来源顺序以提取片名和年份搜索；已知年份不能自动放宽，即使模型提供编号也不省略搜索。来源、电影/节目对象类型和记录编号全部相等时，直接确认对应的真实搜索候选，跳过 Jev 或通用判断模型。编号为空或未命中是正常的未核验提示；当前来源有基本候选时交给所配置判断器，即使只有一个候选也判断，每个有候选来源至多一次。

`Query.Year` 表示输入提取的作品年份；已知时，TMDb 电影查询使用 `primary_release_year`（电影首次发行年份），节目查询使用 `first_air_date_year`（节目首播年份），不追加无年份查询。TheTVDB 服务端年份过滤尚未证实，完整读取标题查询分页后按返回的作品年份核验，未知或异年候选不满足已知年份。搜索缓存操作名改为 `identity-search-v1`，仅隔离旧的忽略/放宽年份结果；详情与 `series-*` 整剧缓存版本保持不变，不迁移或删除旧缓存。
...
两类判断器只接收作品标题、原名、年份、类型和来源作品引用，不接收完整文件清单、季集、分组、单集剧情、图库或演员。作品确认后才批量取得该来源的完整作品与季集事实，输出阶段不再判断；不同本地版本对应同一来源单集时复用事实。
...
单个来源正常空结果、判断拒绝/低分、来源或判断请求/协议错误、选中详情/批量事实缺失，都记录原因后进入下一允许来源；全部网站来源未成功后进入 AI 本地整理，AI 也失败才以元数据获取失败终止。分类输入错误、人工硬约束、用户取消及最终输出错误仍终止。不切换模型、不隐式重试，不把批量错误转为逐集请求。不同来源独立获取完整记录，不逐字段拼接；正常未知可选字段保持空值。

一次运行不驻留、不监听新增文件、不创建定时器。各来源失败在编排边界记录脱敏原因；全部方式均失败或共同执行条件失败时，由运行边界记录最终错误，main 只负责非零退出。已有网站成功作品可以先写入，待本轮网站尝试结束后，对需要本地整理的作品统一批量调用 AI；本地整理失败时不承诺撤销先前文件。

### 数据源无匹配后的 AI 本地整理

该分支处理全部适用网站来源均未取得可用元数据的作品，包括无结果、拒绝/低分和请求/解析/完整性错误。不能预设 res2.txt 全部不存在于网站，也不能跳过已配置且适用的数据源；日志区分无结果与网站故障。人工明确来源或作品约束不能被本地整理绕过。
...
1. 候选阶段仅使用搜索返回的基本电影/节目记录；人工明确作品引用只直取基本记录。作品判断前不请求季集、分组、完整演职员或图片集合。
2. 节目确认后，从节目详情 API 发起组合请求，收集本轮涉及季的单集数组、所需演职员、图库及同对象跨站编号路径，去重后批量追加获取。
...
5. 节目确认后读取所需分组事实，每个不同分组只取一次，再把本地位置映射为官方季集坐标，批取后保留应输出的编排坐标。
...
1. 候选阶段只使用搜索基本节目记录，人工明确引用可直取基本节目记录；确认前不获取全集页或季页面。节目确认后，同一节目页复用一次、全集页取一次，解析各集正式季集号、单集记录编号、标题、简介、日期、电视网及 `data-src` 中的剧照地址。
...
- [ ] 将候选阶段收敛为基本作品记录，确认前季集和完整详情请求为零；选中后才调用现有完整详情与批量接口，并验证原批量字段和请求限额不退化。
...
这一阶段已完成启动参数、配置校验、未分类媒体清单、AI 协议、整剧输出、搜索身份核验和基本候选接口。当前修订三方式接续并重新验证；字段含义以第 3 节为准，来源批量函数仅在作品确认后获取完整事实，不保留可切换的旧退出逻辑。
...
- [x] 提取保留类型、作品及季集映射；两种判断器统一使用 `[]Candidate` 基本作品证据，不携带文件清单、季集、分组、剧情、图库或演员。选中后再接入两来源完整事实和批量函数。
- [x] 模型编号仍先按标题与年份搜索；已知年份不追加无年份查询，TheTVDB 完整分页后按返回年份核验；仅隔离旧搜索缓存，详情与整剧缓存版本不改。只有同来源、同类型编号命中真实候选才零判断确认。未知或未命中时基本候选交给判断器，单候选也判断；人工明确引用仍直取基本记录并判断，不能被模型覆盖。首源确认且完整事实获取成功后，后续来源请求为零。
- [x] 验证网站来源全部未成功后进入本地整理：首源空结果、拒绝、低分、HTTP/协议和详情错误均允许下一适用来源；双网站失败后 AI 成功能输出，AI 也失败才终止。人工指定失败仍不得改用其他来源或 local。
- [x] 移除网络重试配置、退避与重试统计，旧配置字段严格拒绝；不重复同一失败请求。来源错误在来源编排边界脱敏记录并继续，最终终止错误由 collector 记录，main 只负责非零退出。
...
历史主分支提交 `6161a7e` 的复验为：16 个测试包、197 个测试函数、526 个测试节点，race 零失败；vet 与 macOS、Linux amd64/arm64、Windows amd64 构建通过。这是本轮身份与立即失败整改前的基线；新实现必须重新执行本阶段检查，本轮提交 0902136 合并后已完成 16 包、218 个测试函数、594 个节点 race、vet 与四个平台构建复验。本地 NFO 读取边界见专项报告。

- [x] 检查生产源码与调用关系，确认没有运行模式、配置媒体目录、监听/定时调度、预设媒体类型、按样本名字分类、逐视频独立选择节目或默认逐集调用 TMDb 详情的残留。
- [x] 搜索生产代码和生产提示词，确保没有验证标签文件、针对 res2 样本名字的类型分支或“这些样本就是电影”的答案提示；用户答案只存在于隔离的验证期望中。
- [x] 新规则复验请求计数：缓存命中不计外发请求；编号命中后零判断、确认前零季集；首源完整成功后后续零调用，首源未成功后第二源被调用，双网站未成功后 AI 整理被调用。
- [x] 验证观测归属：来源编排边界逐源记录脱敏原因，运行边界记录最终失败，main 只负责退出；纯模型/协议结构不输出日志。确认无凭据泄漏、无 collector/main 重复日志，日志能够区分来源无记录与来源请求故障。
- [x] 单文件原子替换边界不变；首个图片或文件写入失败立即终止整个进程，后续作品零处理；已成功文件保留，不承诺多文件事务回滚。
- [x] 对本轮新实现执行全量 race、vet、平台构建、实际 CLI 受控回归、正式文档与方案本地链接检查；验证通过后记录新证据，不沿用历史结果。README 同步当前契约及仍未覆盖的真实样本与服务器导入边界。
...
- [x] 复核 5,419 个原始视频路径集合，所有占位视频仍为空；本轮初始输入没有复制原有的 5,822 个 NFO、图片、缓存或人工覆盖文件。
- [x] 从执行时的项目 config.json 派生私有测试配置，保留服务地址、模型、凭据、语言、来源顺序、门槛和过滤设置；删除第 3 节列出的旧目录与运行配置，日志指向验证目录。媒体副本目录通过 CLI 参数传入，原配置不由验证器改写。
- [x] 完成本轮固定样本执行：前三部60视频沿用既有证据，按要求暂时跳过第四部104与Friends234，其余五部172均实际运行。每个样本保留真实退出码、失败原因与输出校验，不把暂跳或输出错误计为通过。
- [x] 每部包含目录内所有原始视频，继续应用现有过滤规则。记录“输入 570、按规则排除、实际纳入、已成功、当前失败、尚未处理”的计数，不默认每个输入都应输出 NFO。
- [x] 单次程序失败必须有非零退出码和具体失败阶段；对有效输入未生成 NFO 的情况不得算作通过。匹配结果与目录明显冲突、未知季集、来源数据缺失都保留为失败证据。
- [x] 检查全部生成 XML 的根元素、节目与单集来源归属、季集映射、来源时长及图片文件非空；人工复核每部节目与多季边界、特别篇和非标准文件名。
- [x] 输出脱敏报告，按节目记录来源、提取批次数、候选判断次数、批量请求次数、单集补取次数及原因、纳入文件和 NFO 数量、失败阶段和原因。不得记录凭据或带认证参数的完整 URL。

真实验证使用已配置的来源顺序，不为覆盖另一个 Provider 而改动配置。两种判断器和两个 Provider 的机制覆盖由受控回归保证；真实报告明确本次实际走过哪些服务。

#### B 组：res2.txt 本地内容集合

该组增加本地编目边界验证，不替换A组固定样本。529视频完整复现，忽略5个非视频文件；按7个目录集合与1个独立视频分8轮执行完毕。实际结果包含网站数据、本地整理、布局错误和分类超时，不能预设该组全部不存在于网站。
...
- [x] 完整保留该组原文件名与目录关系，创建空视频副本；忽略清单自身和 4 个脚本，记录清单哈希，不执行脚本。
- [x] 已独立对照实际输出：封箱两分段和独立庆典三个指定样例符合movie预期；年度专场所在196视频集合分类超时，未完成类型验收。预期只交给验证器，未写入生产规则。年度专场、封箱及根目录视频一律先由AI分类。已在忽略目录的 expected-media-types.json 保存 4 个用户确认示例；其中 expected_media_type 定义人工期望的电影/剧集组织类型，只供验证器比较，新增原因是将人工答案与模型输入隔离。它不进入生产规则、目录配置或提示词；AI 判错时报告分类失败，不能事后用文件名特例纠正成预期答案。
- [x] 已运行并核对类型、归组及分支，实际布局错误按失败保留，未修正规则重跑。目标仍为：类型和归组通过校验后，tv 进入整剧批量任务，movie 进入独立作品任务；不能把有编号自动视为剧集或把无编号自动视为电影。网站来源完整成功使用网站事实，全部网站来源未成功后执行受限 AI 本地整理；AI 也失败才终止元数据获取。
- [x] 统计 AI 分类、归组、网站匹配、本地整理、配置/网络失败、输入不足等结果；分类与人工预期对照独立记录。不能把生成 NFO 等同于类型正确或准确匹配，也不能把电影组织形式当作内容一定是院线电影。
- [x] 检查本地 NFO 的标题、季集和描述能追溯到输入；未出现的演员、评分、时长、日期、图片和网站编号必须不存在。本地身份重复运行稳定。
- [x] 增加包含本地唯一标识、无网站编号、无图片及缺少可选事实的 movie/tvshow/episodedetails 样本，按官方文档与源码核查四端识别，不恢复媒体流探测。
...
| 同一轮扫描的两季多集 | AI 归组后形成一个节目任务；作品确认前零季集请求，需判断时每来源至多一次；节目 NFO 一次 |
...
| 已知作品年份 | 搜索不自动放宽；TMDb 使用对应电影/节目年份参数，TheTVDB 完整分页后排除未知或异年候选；旧年份搜索缓存不得复用 |
| 模型编号命中同来源、同类型真实候选 | 先按片名与年份完整搜索，再直接确认；判断器零调用，只为选中作品获取完整详情 |
| 模型编号未知、未命中或与候选来源/类型不同 | 不直取或跨来源命中；当前来源有候选时判断基本作品，输入不携带季集、分组或完整文件清单 |
| 人工明确作品引用 | 直取基本记录并做单候选判断，不被模型提示覆盖；未找到或拒绝均终止 |
| 首个来源确认且完整事实获取成功 | 后续来源零请求，确认后才批量取数，输出保持同一来源 |
| 首个来源搜索正常为空 | 允许下一适用来源；不调用空候选判断 |
| 全部适用网站来源均未取得可用元数据 | 通用 LLM 基于输入批量整理本地元数据；未知事实留空，使用程序生成的本地身份 |
| 判断拒绝或 Jev 低于门槛 | 记录原因后进入下一来源；网站来源耗尽后进入 AI 本地整理 |
| 来源鉴权/网络/协议/详情故障 | 记录该来源原始原因后进入下一允许来源，不重试当前请求 |
| 人工指定来源或作品失败 | 约束保持，不静默更换来源或本地记录 |
| 网站来源和 AI 本地整理全部失败 | 返回非零；日志保留各方式原因，不宣称剩余样本通过；已经写入的文件不回滚 |
...
| 图片或写入阶段失败 | 单文件无半成品，整个进程立即终止；后续作品不处理，已有成功文件保留 |
...
当前方案保留一次性CLI、AI分类、搜索身份核验、基本作品判断、整剧批量及NFO范围。三方式接续已实现并有受控与实际运行证据。本轮非跳过集合已完成执行和输出检查，但节目归组、多集文件、前置条件、候选判断、来源歧义和分类超时仍有待整改或核查事项，整体尚未验收通过；本轮未修改生产规则。下一轮整改需以[问题清单](../../known-issues.md)与实际证据另行确定范围，不能把执行完毕改称验收通过。
